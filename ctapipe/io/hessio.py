...
from ..core.pipeline import Source
...
class HessioSource(Source):

    def __init__(self, url, max_events=None, allowed_tels=None):
        """A Source that streams data from an EventIO/HESSIO MC data file
    ...
    def __iter__(self):
        ...
        container.meta.add_item('hessio__input', self.url)
        container.meta.add_item('hessio__max_events', self.max_events)
        ...
        for run_id, event_id in eventstream:
            ...
            if self.allowed_tels is not None:
                selected = container.dl0.tels_with_data & self.allowed_tels
                if len(selected) == 0:
                    continue  # skip event
            ...
            container.trig.gps_time = Time(
                time_s * u.s, time_ns * u.ns,
            ...
            for tel_id in container.dl0.tels_with_data:
                ...
                for chan in range(nchans):
                    container.dl0.tel[tel_id].adc_samples[chan] = pyhessio.get_adc_sample(
                        channel=chan, telescope_id=tel_id
                    )
                    container.dl0.tel[tel_id].adc_sums[chan] = pyhessio.get_adc_sum(
                        channel=chan, telescope_id=tel_id,
                    )
            ...
            if self.max_events is not None and self.counter > self.max_events:
                raise StopIteration
